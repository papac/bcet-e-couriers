...
%block('title', 'Modifier service - BCET/COURIERS')
...
<div x-data="{ sidebarOpen: false }" class="min-h-screen bg-gray-100">
    ...
    <div class="lg:pl-64 flex flex-col flex-1">
        ...
        <div class="sticky top-0 z-10 flex-shrink-0 flex h-16 bg-white shadow">
            ...
            <div class="flex-1 px-4 flex justify-between items-center">
                <div class="flex items-center">
                    <a href="{{ route('services.index') }}" class="text-gray-500 hover:text-gray-700 mr-4">
                        <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18" />
                        </svg>
                    </a>
                    <h1 class="text-xl font-semibold text-gray-900">Modifier {{ $service->name }}</h1>
                </div>
        ...
        <main class="flex-1 p-6">
            ...
            <form action="{{ route('services.update', ['id' => $service->id]) }}" method="POST" class="max-w-2xl">
                {{ csrf_field() }}
                {{ method_field('PUT') }}
                ...
                <div class="bg-white shadow rounded-xl">
                    <div class="px-6 py-4 border-b border-gray-200">
                        ...
                        <p class="text-sm text-gray-500 mt-1">Code: <span class="font-mono font-medium">{{ $service->code }}</span></p>
                    </div>
                    <div class="p-6 space-y-4">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input type="text" name="city" id="city" required value="{{ $service->city }}"
                                    class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent"
                        ...
                        <div>
                            ...
                            <textarea name="address" id="address" rows="2"
                                class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent"
                                placeholder="Adresse complète du service">{{ $service->address }}</textarea>
                        </div>
                        
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label for="phone" class="block text-sm font-medium text-gray-700 mb-1">Téléphone</label>
                                <input type="tel" name="phone" id="phone" value="{{ $service->phone }}"
                                    class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent"
                                    placeholder="+225 XX XX XX XX XX">
                            </div>
                            <div>
                                <label for="email" class="block text-sm font-medium text-gray-700 mb-1">Email</label>
                                <input type="email" name="email" id="email" value="{{ $service->email }}"
                                    class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent"
                        ...
                        <div>
                            ...
                            <select name="chief_id" id="chief_id"
                                ...
                                <option value="{{ $agent->id }}" {{ $service->chief_id == $agent->id ? 'selected' : '' }}>{{ $agent->name }} ({{ $agent->email }})</option>
                                %endloop
                            </select>
                        </div>
                        ...
                        <div class="flex items-center">
                            ...
                            <label for="is_active" class="ml-2 block text-sm text-gray-700">Service actif</label>
                        </div>
                    </div>
                </div>
                ...
                <div class="mt-6 bg-white shadow rounded-xl">
                    ...
                    <div class="p-6 grid grid-cols-2 md:grid-cols-4 gap-4">
                        <div class="text-center">
                            <p class="text-2xl font-bold text-primary-600">{{ $service->agents()->count() }}</p>
                            <p class="text-sm text-gray-500">Agents</p>
                        </div>
                        <div class="text-center">
                            <p class="text-2xl font-bold text-blue-600">{{ $service->originCouriers()->count() }}</p>
                            <p class="text-sm text-gray-500">Courriers émis</p>
                        </div>
                        <div class="text-center">
                            <p class="text-2xl font-bold text-green-600">{{ $service->destinationCouriers()->count() }}</p>
                            <p class="text-sm text-gray-500">Courriers reçus</p>
                        </div>
                        <div class="text-center">
                            <p class="text-2xl font-bold text-orange-600">{{ $service->currentCouriers()->count() }}</p>
                ...
                <div class="mt-6 flex items-center justify-end space-x-4">
                    <a href="{{ route('services.index') }}" class="px-6 py-2.5 border border-gray-300 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50">
                        Annuler
                    </a>
