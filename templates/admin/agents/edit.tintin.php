...
%block('title', 'Modifier agent - BCET e-Couriers')
        ...
        <div class="sticky top-0 z-10 flex-shrink-0 flex h-16 bg-white shadow">
            ...
            <div class="flex-1 px-4 flex justify-between items-center">
                <div class="flex items-center">
                    ...
                    <h1 class="text-xl font-semibold text-gray-900">Modifier l'agent</h1>
                </div>
            ...
            <div class="max-w-2xl">
                <div class="bg-white shadow rounded-xl">
                    <div class="px-6 py-4 border-b border-gray-200">
                        ...
                        <p class="mt-1 text-sm text-gray-500">Modifiez les informations du compte agent.</p>
                    </div>
                    <form action="/admin/agents/{{ $agent->id }}" method="POST" class="p-6 space-y-6">
                        {{ csrf_field() }}
                        {{ method_field('PUT') }}
                        
                        <div>
                            <label for="name" class="block text-sm font-medium text-gray-700 mb-1">Nom complet *</label>
                            <input type="text" name="name" id="name" required value="{{ $agent->name }}"
                        ...
                        <div>
                            ...
                            <input type="email" name="email" id="email" required value="{{ $agent->email }}"
                                class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent">
                        </div>

                        <div>
                            <label for="phone" class="block text-sm font-medium text-gray-700 mb-1">Téléphone *</label>
                            <input type="tel" name="phone" id="phone" required value="{{ $agent->phone }}"
                                class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent">
                        </div>
                        ...
                        <div class="flex items-center">
                            <input type="checkbox" name="is_active" id="is_active" value="1" {{ $agent->is_active ? 'checked' : '' }}
